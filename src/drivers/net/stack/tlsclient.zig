//! TLS 1.2/1.3 client — GENERATED, DO NOT EDIT BY HAND.
//!
//! Vendored from the Zig standard library's own `std/crypto/tls/Client.zig` as shipped by the
//! pinned toolchain (Zig 0.16.0), then patched for a freestanding kernel by
//! `scripts/build/vendor-tls.py`, which is the only thing that should ever
//! write this file. Re-vendor after a toolchain upgrade with:
//!
//!     scripts/build/vendor-tls.py update    # then READ THE DIFF, then test
//!
//! `scripts/build/vendor-tls.py check` runs in the gate: it regenerates this
//! file and fails when the result differs, so a Zig upgrade cannot quietly
//! leave the kernel on an older TLS implementation.
//!
//! Upstream SHA-256: 1464bd0d53f799e7714fa073a0783cb94c68c10fb1ab4381246e01f9cb130c8a
//!
//! The patches, each anchored on exact upstream text in the script:
//!   - std is a named module here, not a relative path inside the std tree, and the handshake scratch type the caller allocates
//!   - drop the unverified-host arm: kudos always names the host it expects
//!   - drop the unverified/self-signed CA arms, and the host certificate store they are reached through: the bundle is built once at boot and never rescanned
//!   - host switch in init(): no unverified arm to choose
//!   - server-name extension is always present
//!   - cleartext header always carries the server-name extension
//!   - verify the host name unconditionally
//!   - chain verification against the bundle, with no lock to take
//!   - the handshake's record buffers live on the CALLER's heap, not in init's frame: 2 x 16 KiB of cleartext staging is most of a task stack
//!   - generate the key shares OUT OF LINE, so their NTT/field scratch pops before the deep certificate-verification calls instead of coexisting with them for the whole handshake
//!   - the scratch the caller owns, and the option that passes it in
//!   - verify RSA signatures OUT OF LINE. BOTH switches around this are `inline`: ten schemes times four modulus lengths, each instantiating its own key and signature locals, all of which otherwise coexist in verifySignature's single frame — measured at 104408 bytes against a 16384 budget
//!   - the out-of-line RSA verifier itself: one function per scheme, so only that scheme's four modulus instantiations share a frame
//!   - the record-encryption staging belongs to the Client, which the caller heaps, not to a frame: 16 KiB of cleartext buffer sits just over the budget on its own
//!   - initialise that staging in the returned Client (its contents are written before each use)
//!   - the Client field that staging lives in
//!   - no host certificate store to fall back to: an unverified chain is the end of it
//!
//! THE STACK IS 128 KiB, not a host thread's megabytes: the handshake's record
//! staging is a caller-owned `HandshakeScratch` (heap) rather than locals in
//! `init`, and the key-share generation is called out of line so its scratch
//! pops before certificate verification runs. scripts/tests/stackframes.sh is
//! what holds that line.
//!
//! THERE IS NO UNAUTHENTICATED MODE (spec NET-011). Upstream's no-verification
//! and self-signed arms are deleted rather than merely unused, so a bypass does
//! not compile. The wall clock the certificate dates are checked against comes
//! from the caller (`Options.realtime_now`), which is how a kernel with no
//! `std.time` supplies it.
...
/// The handshake's record staging, owned by the CALLER so it lands on the heap:
/// these two buffers are 32 KiB of a 128 KiB kernel task stack if left as locals
/// in `init`, which is what upstream does (see the file header).
pub const HandshakeScratch = struct {
    cleartext_bufs: [2][std.crypto.tls.max_ciphertext_inner_record_len]u8,
};
...
/// Record-encryption staging for `prepareCiphertextRecord` (see the file
/// header): upstream keeps this in that function's frame, where 16 KiB alone
/// exceeds the kernel's per-frame budget. A POINTER to caller-owned memory, not
/// the buffer itself — `init` returns a Client by value, so an inline buffer
/// would simply move the same 16 KiB into the caller's frame instead.
cleartext_scratch: *[max_ciphertext_len]u8,
...
pub const Options = struct {
    ...
    host: union(enum) {
        ...
        /// The unverified arm upstream offers is deleted (see the file header).
        explicit: []const u8,
    },
    /// How to verify the authenticity of server certificates.
    /// Verify that the server certificate is authorized by this bundle. It is
    /// the ONLY mode: the unverified and self-signed arms upstream offers are
    /// deleted (see the file header). Upstream reaches its bundle through an
    /// `Io` and an `RwLock` so it can rescan the host's certificate store; the
    /// kernel's bundle is parsed once from the embedded roots and never
    /// rescanned, so it is named directly.
    ca: struct { bundle: *Certificate.Bundle },
    write_buffer: []u8,
    read_buffer: []u8,
    ...
    /// Handshake staging the CALLER owns (see the file header): upstream keeps
    /// these buffers in `init`'s frame, which no kernel task stack can carry.
    handshake_scratch: *HandshakeScratch,
    /// Record-encryption staging the CALLER owns, for the same reason: every
    /// outgoing record is assembled here before it is encrypted.
    record_scratch: *[std.crypto.tls.max_ciphertext_len]u8,
...
pub fn init(input: *Reader, output: *Writer, options: Options) InitError!Client {
    ...
    var key_share = @call(.never_inline, KeyShare.init, .{options.entropy[64..240]}) catch |err| switch (err) {
    ...
    const cleartext_bufs = &options.handshake_scratch.cleartext_bufs;
    fragment: while (true) {
                ...
                switch (handshake_type) {
                    ...
                    .certificate => cert: {
                        ...
                        while (!certs_decoder.eof()) {
                            ...
                            if (options.ca.bundle.verify(subject, now_sec)) {
                    ...
                    .finished => {
                        ...
                        return .{
                            .cleartext_scratch = options.record_scratch,
                            .input = input,
                            .reader = .{
...
fn prepareCiphertextRecord(
    ...
    // It lives in caller-owned memory rather than this frame.
    const cleartext_buf = c.cleartext_scratch;
    var ciphertext_end: usize = 0;
    var bytes_i: usize = 0;
...
const CertificatePublicKey = struct {
    ...
    /// One scheme's RSA verification, in its OWN frame (see the file header).
    /// Kept out of `verifySignature` so the inline scheme x modulus-length
    /// expansion does not stack every instantiation's locals together.
    fn verifyRsa(
        ...
        const RsaSignature = SchemeRsa(scheme);
        const Hash = SchemeHash(scheme);
        ...
        switch (modulus.len) {
            inline 128, 256, 384, 512 => |modulus_len| {
                ...
                try @call(.never_inline, RsaSignature.concatVerify, .{ modulus_len, sig, msg, key, Hash });
            },
    ...
    fn verifySignature(
        ...
        switch (scheme) {
            ...
            .rsa_pkcs1_sha1,
            => |comptime_scheme| {
                try @call(.never_inline, verifyRsa, .{ comptime_scheme, pub_key, encoded_sig, msg });
            },
...
/// Upstream falls back to the operating system's certificate store when the
/// bundle does not establish trust (`Certificate.Chain`, Windows only — it is
/// `void` everywhere else, so this was already dead code off that platform).
/// A kernel has no such store: the embedded roots are the whole trust anchor.
