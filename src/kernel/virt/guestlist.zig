...
/// The catalog: the three guests kudos boots, in list order (entry 1 is the
...
/// these three are built for this hypervisor and each does a job.
///
...
/// console, a virtio-gpu scanout, and a virtio-net card whose frames reach the
/// wire through the bridge (VIRT-027) — and no disk at all (VIRT-004) — so an
/// initramfs that is only a first stage has nowhere to fetch its second stage
/// from. Alpine's own netboot pair is the example worth remembering: its kernel
...
pub const CATALOG = [_]Image{
    .{
        ...
        // ~630 MiB of root filesystem in tmpfs, and Firefox's own working set
