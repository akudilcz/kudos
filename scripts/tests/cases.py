...
CASES = [
    ...
    # ── option grammar: flags cluster, values attach, unknowns refuse ────
    # APP-033: a command that shares a Linux command's name takes its option
    # letters, with the same meaning. APP-034: one it does not implement is
    # refused by name, with the usage — never run as if the flag were absent.
    Case("ls -al /ramdisk", ("-   456024 teapot.glb",), "both"),
    Case("ls -lh /ramdisk", ("-     445K teapot.glb",), "both"),
    Case("ls -Z", ("ls: invalid option -- 'Z'", "usage: ls [-alh] [PATH...]"), "both"),
    Case("echo -n ten-chars- | wc -c", ("      10",), "both"),
    Case("echo Mixed-Case | grep -i mixed", ("Mixed-Case",), "both"),
    Case("echo one two | grep -v absent | wc -l", ("      1",), "both"),
    Case("echo 12345 | head -c 3", ("123",), "both"),
    Case("cat -n redir.txt", ("     1  redirect-one", "     2  redirect-two"), "both"),
    Case("mkdir -p deep/er/est", (), "both"),
    Case("ls deep/er", ("est/",), "both"),
    Case("rm -r deep", (), "both"),
    Case("ls deep", ("ls: no such directory",), "both"),

    # ── the text tools: each reads a named file or the pipe (filter.zig), and
    # each takes the option letters its Linux namesake takes ──────────────
    Case("seq 1 5 > seq.txt", (), "both"),
    Case("tail -n 2 seq.txt", ("4", "5"), "both"),
    Case("seq 1 5 | tail -n 1", ("5",), "both"),
    Case("head -n 1 seq.txt", ("1",), "both"),
    Case("seq -s , 1 3", ("1,2,3",), "both"),
    Case("nl -b a seq.txt", ("     1\t1", "     5\t5"), "both"),
    Case("sort -r seq.txt | head -n 1", ("5",), "both"),
    Case("echo b > sort.txt; echo a >> sort.txt", (), "both"),
    Case("sort sort.txt", ("a", "b"), "both"),
    Case("echo dup > dup.txt; echo dup >> dup.txt", (), "both"),
    Case("uniq -c dup.txt", ("      2 dup",), "both"),
    Case("echo one:two:three | cut -d : -f 2", ("two",), "both"),
    Case("echo abcdef | cut -c 2-4", ("bcd",), "both"),
    Case("echo teed | tee teed.txt", ("teed",), "both"),
    Case("cat teed.txt", ("teed",), "both"),
    Case("diff seq.txt seq.txt", (), "both"),
    Case("diff -q seq.txt sort.txt", ("differ",), "both"),
    Case("echo hex | xxd", ("00000000: 6865 780a", "hex."), "both"),
    Case("echo hex | xxd -p", ("6865780a",), "both"),
    Case("more seq.txt", ("5 lines",), "both"),
    Case("less seq.txt", ("5 lines",), "both"),
    # An unimplemented option is refused BY NAME, never ignored: a tool that
    # silently drops a flag answers a question nobody asked.
    Case("sort -Z", ("sort: invalid option -- 'Z'", "usage: sort"), "both"),
    Case("tail -f seq.txt", ("-f has nothing to follow",), "both"),

    # ── the file-system tools, over one shared walk (fstree.zig) ──────────
    Case("mkdir -p tree/inner", (), "both"),
    Case("echo x > tree/inner/leaf.txt", (), "both"),
    Case("find tree", ("tree/inner", "tree/inner/leaf.txt"), "both"),
    Case("find tree -name '*.txt'", ("leaf.txt",), "both"),
    Case("find tree -iname '*.TXT'", ("leaf.txt",), "both"),
    Case("find tree -type d", ("tree/inner",), "both"),
    Case("find tree -bogus x", ("find: unknown predicate: -bogus",), "both"),
    Case("stat -c %s tree/inner/leaf.txt", ("2",), "both"),
    Case("stat tree/inner/leaf.txt", ("Type: regular file", "Lines: 1"), "both"),
    Case("du -s tree", ("tree",), "both"),
    Case("df", ("Mount", "/ramdisk"), "both"),
    Case("basename /a/b/c.zig .zig", ("c",), "both"),
    Case("dirname /a/b/c.zig", ("/a/b",), "both"),
    Case("rm -r tree", (), "both"),

    # ── `;` command lists, and the quotes that protect text from them ────
    Case("echo first-of-two; echo second-of-two", ("first-of-two", "second-of-two"), "both"),
    Case("echo 'const a = 1;' > semi.txt", (), "both"),
    Case("cat semi.txt", ("const a = 1;",), "both"),

    # APP-032: the ONE in-flight transfer slot refuses a second curl with the
    # holder's work undisturbed. The `;` list makes the collision: the first
    # curl backgrounds (prompt held), the second lands while it is in flight.
    Case("curl http://10.0.2.2:9/x ; curl http://10.0.2.2:9/y",
         ("curl: a transfer is already in flight",), "emulated"),
    ...
    Case("cat", ("usage: cat FILE...",), "both"),
    Case("cat /ramdisk/nope.txt", ("cat: no such file '/ramdisk/nope.txt'",), "both"),
    Case("ls /nope", ("ls: no such directory '/nope'",), "both"),
    Case("ls /ramdisk/motd.txt", ("motd.txt  (",), "both"),
